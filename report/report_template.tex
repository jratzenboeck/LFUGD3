...
To improve your existing matrix factorization approach we used two adaptions of the matrix factorization based on the he Singular Value Decomposition (SVD).
\newline
\newline
\textbf{\emph{Singular Value Decomposition++ (SVD++) }}
\newline

This approach is an extension to SVD-based latent factor models. It improves the accuracy by using explicit feedback and the implicit feedback from users. Through this the preferences of a user are represented by a combination of the explicit and implicit information.
\newline
In this case the explicit feedback of a user is based on the movie ratings he made and the implicit feedback is based on the movies a user rated, without consideration of the rating.
The following equation shows the SVD++ model:
...
$ p_{u} $ represents the user with the implicit feedback
...
$ b_{ui} $ represents the bias of a user item tupel
\newline
\newline
\textbf{\emph{gSingular Value Decomposition++ (gSVD++) }}
\newline

This approach exploits implicit and explicit feedback from users and the information of the available metadata associated to the items. This way it enriches the SVD++ about the information associated to the items, like genres, actors and keywords.
\newline
Therefore, the model of the SVD++ is enhanced with the implicit feedback for the item factors.
The following equation shows the adaption by the gSVD++ :
...
\newline
\newline
For the gSVD++ approach we was not able to compute any reasonable results. The reason for this was that, if our lists with the movies and its associated attributes has reached a certain size the script stopped with an out of memory exception.
